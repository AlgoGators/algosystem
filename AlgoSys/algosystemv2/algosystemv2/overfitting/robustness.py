...
class SharpeCI:
    ...
    def report(self):
        ...
        print(f"Sharpe Ratio: {self.point_estimate:.4f}")
        print(f"  {self.confidence:.0%} CI: [{self.ci_lower:.4f}, {self.ci_upper:.4f}]")
        print(f"  Includes zero: {self.ci_includes_zero} [{status}]")
...
class MonteCarloResults:
    ...
    def report(self):
        print("Monte Carlo Trade Resampling")
        print(f"  Trades: {self.n_trades}, Simulations: {self.n_simulations}")
        print(f"  Terminal PnL:  actual={self.actual_terminal_pnl:.4f}")
        print(f"    MC 5-95%:    [{self.terminal_pnl_5th:.4f}, {self.terminal_pnl_95th:.4f}]")
        print(f"    Percentile:  {self.terminal_pnl_percentile:.1f}th")
        print(f"  Max Drawdown:  actual={self.actual_max_drawdown:.4f}")
        print(f"    MC 5-95%:    [{self.max_drawdown_5th:.4f}, {self.max_drawdown_95th:.4f}]")
        print(f"    Percentile:  {self.max_drawdown_percentile:.1f}th")
        if self.actual_max_drawdown < self.max_drawdown_5th:
            print("  WARNING: Actual drawdown was unusually mild -- may have been lucky")
        if self.actual_max_drawdown > self.max_drawdown_95th:
            print("  WARNING: Actual drawdown was unusually severe")
...
def monte_carlo_trades(
    ...
    if n_trades < 3:
        return MonteCarloResults(
            ...
            actual_terminal_pnl=float(np.sum(pnls)), actual_max_drawdown=0.0,
            terminal_pnl_mean=0, terminal_pnl_5th=0, terminal_pnl_95th=0,
            max_drawdown_mean=0, max_drawdown_5th=0, max_drawdown_95th=0,
            terminal_pnl_percentile=50, max_drawdown_percentile=50,
        )
...
class AlphaBetaResult:
    ...
    def report(self):
        ...
        print(f"Alpha/Beta Decomposition [{verdict}]")
        print(f"  Alpha (annual): {self.alpha_annual:.4f} ({self.alpha_annual*100:.2f}%)")
        print(f"  Beta:           {self.beta:.4f}")
        print(f"  R-squared:      {self.r_squared:.4f}")
        print(f"  Info Ratio:     {self.information_ratio:.4f}")
        print(f"  Tracking Error: {self.tracking_error:.4f}")
        if self.is_just_beta:
            print("  WARNING: Strategy returns are mostly explained by market exposure")
...
class KellyResult:
    ...
    def report(self):
        ...
        print(f"Kelly Criterion [{verdict}]")
        print(f"  Win rate:       {self.win_rate:.1%}")
        print(f"  Avg win:        {self.avg_win:.6f}")
        print(f"  Avg loss:       {self.avg_loss:.6f}")
        print(f"  Payoff ratio:   {self.payoff_ratio:.2f}")
        print(f"  Kelly fraction: {self.kelly_fraction:.4f} ({self.kelly_fraction*100:.1f}%)")
        print(f"  Half-Kelly:     {self.half_kelly:.4f} ({self.half_kelly*100:.1f}%)")
        print(f"  Edge (EV/$):    {self.edge:.6f}")
        if not self.has_edge:
            print("  WARNING: Kelly fraction <= 0: no edge to exploit")
        else:
            print(f"  Risk of ruin (full Kelly):  {self.risk_of_ruin_full_kelly:.4f}")
            print(f"  Risk of ruin (half Kelly):  {self.risk_of_ruin_half_kelly:.4f}")
...
def kelly_criterion(
    ...
    if len(pnls) < 5:
        return KellyResult(
            ...
            kelly_fraction=0, half_kelly=0, edge=0,
            risk_of_ruin_full_kelly=1, risk_of_ruin_half_kelly=1,
            has_edge=False,
        )
...
class RegimeResult:
    ...
    def report(self):
        ...
        print(f"Regime-Conditional Performance [{verdict}]")
        print(f"  Overall Sharpe: {self.overall_sharpe:.4f}")
        print(f"  {'Regime':<15} {'Sharpe':>8} {'Frac':>8} {'Days':>6}")
        print(f"  {'-'*15} {'-'*8} {'-'*8} {'-'*6}")
        for i, name in enumerate(self.regime_names):
            print(f"  {name:<15} {self.regime_sharpes[i]:8.4f} "
                  f"{self.regime_frac[i]:7.1%} {self.regime_counts[i]:6d}")
        if not self.works_in_all_regimes:
            print(f"  WARNING: Worst regime: {self.worst_regime_name} "
...
def regime_conditional_performance(
    ...
    if n <= vol_window:
        ...
        return RegimeResult(
            regime_names=['all'], regime_sharpes=[0.0], regime_counts=[n],
            regime_frac=[1.0], overall_sharpe=0.0, worst_regime_sharpe=0.0,
            worst_regime_name='all', works_in_all_regimes=False,
        )
    ...
    regime_names = ['low_vol', 'med_vol', 'high_vol'] if n_regimes == 3 else \
