...
def compute_return_stats(
    ...
    if sigma < 1e-15:
        return ReturnStats(
            n_obs=n, mean=mu, std=sigma, sharpe=0.0,
            skewness=0.0, kurtosis=3.0, acf1=0.0, n_effective=n,
        )
    ...
    return ReturnStats(
        n_obs=n, mean=mu, std=sigma, sharpe=sharpe,
        skewness=gamma3, kurtosis=gamma4, acf1=acf1, n_effective=n_eff,
    )
...
class PSRResult:
    ...
    def report(self):
        ...
        print(f"Probabilistic Sharpe Ratio [{status}]")
        print(f"  PSR             : {self.psr:.4f} (threshold: {self.threshold:.2f})")
        print(f"  Observed SR     : {self.sr_hat:.4f}")
        print(f"  Benchmark SR0   : {self.sr0:.4f}")
        print(f"  Test statistic  : {self.test_statistic:.4f}")
        print(f"  Observations    : {self.n_obs} (effective: {self.n_effective})")
...
class DSRResult:
    ...
    def report(self):
        ...
        print(f"Deflated Sharpe Ratio [{status}]")
        print(f"  DSR             : {self.dsr:.4f} (threshold: {self.threshold:.2f})")
        print(f"  Best observed SR: {self.sr_hat:.4f}")
        print(f"  SR0* (haircut)  : {self.sr0_star:.4f}")
        print(f"  Trials tested   : {self.n_trials} (effective: {self.n_trials_effective})")
        print(f"  Min track record: {self.min_track_record:.0f} observations")
        print(f"  Test statistic  : {self.test_statistic:.4f}")
        print(f"  Skewness        : {self.skewness:.4f}")
        print(f"  Kurtosis        : {self.kurtosis:.4f}")
...
class BatchDSRResult:
    ...
    def report(self):
        print("=" * 72)
        print(f"BATCH DEFLATED SHARPE RATIO ({self.n_strategies} strategies)")
        print("=" * 72)
        print(f"Significant (DSR > 0.95): {self.n_significant}/{self.n_strategies}")
        print(f"Best DSR: {self.best_dsr:.4f} (strategy #{self.best_strategy_idx})")
        print()
        print(f"{'#':>3}  {'SR_hat':>8}  {'SR0*':>8}  {'DSR':>8}  {'Sig?':>5}  {'MinTRL':>8}")
        print("-" * 52)
        for i, r in enumerate(self.results):
            ...
            print(f"{i:3d}  {r.sr_hat:8.4f}  {r.sr0_star:8.4f}  "
...
def batch_deflated_sharpe(
    ...
    all_sharpes = np.array([
        compute_return_stats(r, annualize).sharpe for r in returns_list
    ])
    ...
    return BatchDSRResult(
        ...
        summary_table=[
            {'strategy': i, 'sr': r.sr_hat, 'dsr': r.dsr, 'significant': r.is_significant}
...
class TrialTracker:
    ...
    Usage:
        ...
        for params in param_grid:
            ...
            print(f"DSR after {tracker.n_trials} trials: {record.dsr:.4f}")
    """
    ...
    def record_trial(
        ...
        psr_result = probabilistic_sharpe_ratio(
            returns, sr0=0.0, annualize=self.annualize,
        )
    ...
    def report(self):
        """Print current trial tracker status."""
        print("=" * 72)
        print(f"TRIAL TRACKER ({self.n_trials} trials recorded)")
        print("=" * 72)
        if self.n_trials == 0:
        ...
        print(f"  Current SR0* threshold : {sr0:.4f}")
        print(f"  Best observed Sharpe   : {best.sharpe:.4f} (trial #{best.trial_id})")
        print(f"  Survivors (DSR > {self.base_threshold}): {n_surv}/{self.n_trials}")
        print()
        print(f"  {'Trial':>5}  {'SR':>8}  {'PSR':>6}  {'DSR':>6}  {'Sig?':>5}  Strategy")
        print(f"  {'-'*5}  {'-'*8}  {'-'*6}  {'-'*6}  {'-'*5}  {'-'*15}")
        for t in sorted(self.trials, key=lambda t: -t.sharpe)[:20]:
            sig = "YES" if t.significant_at_time else "no"
            print(f"  {t.trial_id:5d}  {t.sharpe:8.4f}  {t.psr:6.3f}  "
