...
class OverfitResults:
    ...
    def report(self):
        ...
        if self.returns is not None:
            try:
                ...
                print(f"Analytical DSR (Bailey/LdP)    : {adsr['dsr']:.4f}")
                print(f"  SR0 (haircut threshold)      : {adsr['sr0']:.4f}")
                print(f"  Min track record length      : {adsr['min_trl']:.1f}")
                print(f"  Returns skewness             : {adsr['skewness']:.4f}")
                print(f"  Returns kurtosis             : {adsr['kurtosis']:.4f}")
            except Exception:
                pass
        print()
    ...
    def analytical_deflated_sharpe(self, returns: np.ndarray | None = None) -> dict:
        """
        ...
        if ret is None:
            raise ValueError(
                "Returns array required for analytical DSR. "
                "Pass returns= or set self.returns."
            )
        ...
        min_trl = self.min_track_record_length(
            target_dsr=0.95, _sr0=sr0, _gamma3=gamma3, _gamma4=gamma4,
        )
