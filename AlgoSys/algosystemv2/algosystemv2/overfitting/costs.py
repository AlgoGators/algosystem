...
def apply_transaction_costs(
    ...
    n = len(returns)
    if len(positions) != n:
        raise ValueError(f"returns ({n}) and positions ({len(positions)}) must have same length")
...
def wrap_backtest_with_costs(
    ...
    For the built-in strategies, use make_cost_aware_backtest() instead.
    """
    from ._utils_costs import sharpe_net_of_costs
    return lambda params, returns: sharpe_net_of_costs(
        backtest_fn, params, returns, cost_model
    )
