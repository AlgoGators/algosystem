...
class StepwiseResults:
    ...
    def report(self):
        """Print stepwise test results."""
        print("=" * 72)
        print("STEPWISE PERMUTATION TEST (Romano-Wolf)")
        print("=" * 72)
        print(f"Competitors tested   : {self.n_params}")
        print(f"Permutation reps     : {self.n_reps}")
        print(f"FWE alpha            : {self.alpha}")
        print(f"Nulls rejected       : {self.n_rejected}")
        print()

        n_show = min(20, self.n_params)
        print(f"Top {n_show} parameter sets (stepwise order):")
        print("-" * 72)
        print(f"{'Rank':>4}  {'Sharpe':>8}  {'Step pval':>10}  {'Rejected':>8}  Params")
        print("-" * 72)
...
def stepwise_permutation_test(
    ...
    counts = np.ones(n_params, dtype=np.int64)  # start at 1 (Phipson & Smyth)
