...
class WalkForwardResults:
    ...
    def report(self):
        ...
        print(f"Purge gap            : {self.purge_gap} periods")
        print()
        print(f"Walk-Forward Eff.    : {self.wfe:.4f}  [{verdict}]")
        print(f"Mean IS Sharpe       : {self.mean_is_sharpe:.4f}")
        print(f"Mean OOS Sharpe      : {self.mean_oos_sharpe:.4f}")
        print(f"Folds OOS > 0        : {self.frac_oos_positive:.1%}")
        if self.vetoed:
            print(f"CATASTROPHIC VETO    : YES — folds {self.catastrophic_folds}")
        print()
        print("Per-fold breakdown:")
        print("-" * 72)
        print(f"{'Fold':>4}  {'IS Sharpe':>10}  {'OOS Sharpe':>11}  {'Degrad':>8}  Best params")
        print("-" * 72)
        for i in range(self.n_folds):
            dr = self.degradation_ratios[i]
            dr_str = f"{dr:.2f}" if np.isfinite(dr) else "N/A"
            print(f"{i+1:4d}  {self.is_sharpes[i]:10.4f}  "
...
def walk_forward_analysis(
    ...
    fold_size = n // n_folds
    if fold_size < 20:
        raise ValueError(f"Fold size {fold_size} too small (n={n}, folds={n_folds})")
