...
class TestSurfaceAnalysisStructure:
    def test_returns_dict(self, noise_returns, medium_grid):
        det = OverfitDetector(
            backtest_fn=backtest_noise, returns=noise_returns,
            param_grid=medium_grid, n_reps=50, n_workers=1, seed=42,
        )
        res = det.run()
    ...
    def test_expected_keys(self, noise_returns, medium_grid):
        ...
        res = det.run()
        sa = res.surface_analysis()
        for k in ['robustness_ratio', 'frac_positive', 'frac_above_half',
...
class TestPlateauVsSpike:
    def test_flat_surface(self):
        ...
        det = OverfitDetector(
            backtest_fn=flat_backtest, returns=flat_returns,
    ...
    def test_spiky_worse_than_flat(self):
        ...
        sa_flat = OverfitDetector(
            backtest_fn=flat_backtest, returns=flat_returns,
            param_grid=grid, n_reps=20, n_workers=1, seed=42,
        ).run().surface_analysis()

        sa_spiky = OverfitDetector(
            backtest_fn=spiky_backtest, returns=flat_returns,
            param_grid=grid, n_reps=20, n_workers=1, seed=42,
        ).run().surface_analysis()
...
class TestSobolDominantParam:
    def test_lookback_dominates(self):
        ...
        det = OverfitDetector(
            backtest_fn=lookback_dependent, returns=returns,
