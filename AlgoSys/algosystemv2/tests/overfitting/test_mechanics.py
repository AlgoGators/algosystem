...
class TestSubsampling:
    def test_correct_param_count(self, noise_returns):
        det = OverfitDetector(
            ...
            n_reps=50, max_param_trials=5, n_workers=1, seed=42,
        )
        res = det.run()
...
class TestEdgeCases:
    ...
    def test_constant_returns_no_crash(self):
        ...
        det = OverfitDetector(
            backtest_fn=backtest_noise, returns=const_returns,
...
class TestComparisonDirection:
    def test_best_is_max(self, noise_returns, small_grid):
        det = OverfitDetector(
            backtest_fn=backtest_noise, returns=noise_returns,
            param_grid=small_grid, n_reps=100, n_workers=1, seed=42,
        )
        res = det.run()
...
class TestReproducibility:
    def test_same_seed_same_results(self, noise_returns, small_grid):
        kwargs = dict(
            backtest_fn=backtest_noise, returns=noise_returns,
            param_grid=small_grid, n_reps=50, n_workers=1, seed=42,
        )
...
class TestMultiprocessing:
    def test_single_vs_multi(self, noise_returns, small_grid):
        kwargs_base = dict(
            backtest_fn=backtest_noise, returns=noise_returns,
            param_grid=small_grid, n_reps=50, seed=42,
        )
...
class TestNullDistributionShape:
    def test_correct_length(self):
        ...
        det = OverfitDetector(
            ...
            n_reps=300, n_workers=1, seed=42,
        )
        res = det.run()
