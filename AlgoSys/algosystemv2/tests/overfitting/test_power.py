...
class TestPowerRealSignal:
    ...
    def test_low_pvalue(self, signal_returns, small_grid):
        det = OverfitDetector(
            backtest_fn=backtest_noise, returns=signal_returns,
            param_grid=small_grid, n_reps=200, n_workers=1, seed=42,
        )
        res = det.run()
...
class TestDeflatedSharpe:
    ...
    def test_signal_beats_noise(self):
        ...
        res_noise = OverfitDetector(
            backtest_fn=backtest_noise, returns=noise,
            param_grid=grid, n_reps=300, n_workers=1, seed=42,
        ).run()
        res_signal = OverfitDetector(
            backtest_fn=backtest_noise, returns=signal,
            param_grid=grid, n_reps=300, n_workers=1, seed=42,
        ).run()
