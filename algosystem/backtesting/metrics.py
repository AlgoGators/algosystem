...
def calculate_time_series_data(strategy, benchmark=None, window=30):
    ...
    time_series['monthly_returns'] = strategy_returns.resample('ME').apply(lambda x: (1 + x).prod() - 1)
    time_series['yearly_returns'] = strategy_returns.resample('YE').apply(lambda x: (1 + x).prod() - 1)
    
    # Calculate rolling returns for different periods
    time_series['rolling_3m_returns'] = (1 + strategy_returns).rolling(63).apply(lambda x: x.prod() - 1, raw=True)
    time_series['rolling_6m_returns'] = (1 + strategy_returns).rolling(126).apply(lambda x: x.prod() - 1, raw=True)
    time_series['rolling_1y_returns'] = (1 + strategy_returns).rolling(252).apply(lambda x: x.prod() - 1, raw=True)
...
def calculate_metrics(strategy, benchmark=None):
    ...
    if benchmark_returns is not None:
        try:
            ...
            metrics['information_ratio'] = qs.stats.information_ratio(strategy_returns, benchmark_returns)
            metrics['capture_ratio_up'] = qs.stats.capture(strategy_returns, benchmark_returns, up=True)
            metrics['capture_ratio_down'] = qs.stats.capture(strategy_returns, benchmark_returns, up=False)
        except Exception as e:
