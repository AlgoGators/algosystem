...
class AlgoSystem:
    ...
    def export_charts(engine, output_dir=None, dpi=300):
        ...
        if 'equity' in results:
            output_paths.append(save_chart(
                results['equity'], 
        ...
        for name, data in plots.items():
            ...
            if name in ['drawdown_series', 'rolling_sharpe', 'rolling_sortino', 
    ...
    def list_configs():
        """
        List all available configuration files in the user directory.
        ...
        if not os.path.exists(user_config_dir):
            logger.info("No configuration directory found")
            return []
        
        config_files = [f for f in os.listdir(user_config_dir) if f.endswith('.json')]
        
        console.print(f"[bold blue]Configuration files in {user_config_dir}:[/bold blue]")
        for i, config_file in enumerate(config_files):
            ...
            console.print(f"[cyan]{i+1}.[/cyan] {config_file} [gray]({file_size:.1f} KB, modified: {mod_time_str})[/gray]")
